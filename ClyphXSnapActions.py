...
class ClyphXSnapActions(ControlSurfaceComponent):
	...
	if track_list:
		...
		if not track_name.startswith('CLYPHX SNAP') and not snap_data.has_key(track.name):
		    ...
		    if 'MIX+' in args and track != self.song().master_track:
	...
	self._snap_id = xclip.name[xclip.name.index('['):xclip.name.index(']')+1].strip() 
	...
	if self._is_control_track:
		...
		try: smooth_speed = int(track_name[track_name.index('[')+1:track_name.index(']')].strip())
		except: smooth_speed = None
		if smooth_speed != None and smooth_speed in range(100):
		    self._smoothing_speed = smooth_speed
	self._smoothing_active = False
	self._rack_smoothing_active = False
	self._parameters_to_smooth = {}
	self._rack_parameters_to_smooth = {}
	for track, param_data in snap_data.items():
		...
		if param_data[0]:
		    if track.mixer_device.volume.is_enabled:
			self.get_parameter_data_to_smooth(track.mixer_device.volume, param_data[0][0])
		    if track.mixer_device.panning.is_enabled:
	...
	if self._is_control_track and self._parameters_to_smooth and self._smoothing_speed and ((not self._control_rack) or (self._control_rack and not self._control_rack.parameters[0].value == 1.0)):
	    self._smoothing_active = True
	elif self._is_control_track and self._parameters_to_smooth and self._control_rack:
    ...
    def on_timer(self):
	""" Smooth parameter value changes """
	if self._smoothing_active and self._parameters_to_smooth:
	    for p, v in self._parameters_to_smooth.items():
		target_reached = False
		new_value = p.value + v[0]
		if new_value >= p.min and new_value <= p.max:
		    if (v[0] > 0 and p.value < v[1]) or (v[0] < 0 and p.value > v[1]):
			if (v[0] > 0 and new_value <= v[1]) or (v[0] < 0 and new_value >= v[1]):
			    p.value = new_value
			else:
			    target_reached = True
		    else:
	...
	if factor and self._is_control_track:
	    ...
	    if difference and (factor == 127 or (factor != 127 and abs(difference) > 0.1)):
	...
	for track in (self.song().tracks + self.song().return_tracks + (self.song().master_track,)):
    ...
    def disconnect(self): 
	if self._has_timer:
	    self._parent._unregister_timer_callback(self.on_timer)
